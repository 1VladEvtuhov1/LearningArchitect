# LearningArchitect (Unity Client)

This folder is the **Unity project** for ExtractionRPG. Open it in Unity Hub — not the repository root.
...
Module and variant **wiring** (`ModuleDefinitionSO` / `VariantDefinitionSO`) lives under `Assets/Content/Modules/*/Data` and holds `localizationKey`, taxonomy, prefab references, and stress counts. **All visible prose** is authored in Unity Localization:
...
The full content rules live in `Architecture.md` (this folder).
...
- `Assets/Content/Showcase/Prefabs/ArchitectureShowcaseHub.prefab` is the runtime UI source of truth. `Assembly-CSharp.csproj` is Unity-generated and should not be read as an architecture document.
