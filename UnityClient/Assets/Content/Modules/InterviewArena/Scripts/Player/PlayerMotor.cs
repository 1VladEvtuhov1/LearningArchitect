...
namespace LearningArchitect.Modules.InterviewArena
{
    ...
    [RequireComponent(typeof(PlayerDashIframeGuard))]
    ...
    public sealed class PlayerMotor : MonoBehaviour
    {
        ...
        public bool IsDashIframeActive
        {
            get
            {
                if (!IsDashing || config == null)
                    return false;

                float duration = config.DashDuration;
                float elapsed = duration - dashTimer;
                return DashIframeRules.IsActive(elapsed, duration, config.DashIframeCoverage);
            }
        }
        ...
        public void ApplyConfig(PlayerConfig playerConfig, Camera camera)
        {
            ...
            if (TryGetComponent(out MeleeBlockController meleeBlock))
        ...
        private void ApplyPlanarMovement(Vector2 moveInput)
        {
            ...
            {
                if (meleeStrike != null && meleeStrike.IsStrikeStepActive)
                    return;
