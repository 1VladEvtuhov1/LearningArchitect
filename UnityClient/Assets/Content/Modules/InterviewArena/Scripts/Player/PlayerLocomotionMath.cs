...
namespace LearningArchitect.Modules.InterviewArena
{
    public static class PlayerLocomotionMath
    {
        ...
        public static Vector3 SlideAlongWall(Vector3 planarVelocity, Vector3 wallNormal)
        {
            ...
            normal.Normalize();
            float intoWall = Vector3.Dot(planarVelocity, normal);
            if (intoWall <= 0f)
                return planarVelocity;

            return planarVelocity - normal * intoWall;
        }

        public static float ApplyJumpCut(float verticalVelocity, float cutMultiplier)
        {
            if (verticalVelocity <= 0f)
                return verticalVelocity;

            return verticalVelocity * Mathf.Clamp01(cutMultiplier);
        }
